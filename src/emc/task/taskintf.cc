...
{
    CATCH_NAN(rtapi_isnan(backlash));
...
int emcJointSetInputScale(int joint, double input_scale)
{
    CATCH_NAN(rtapi_isnan(input_scale));

    if (joint < 0 || joint >= EMCMOT_MAX_JOINTS) {
        return 0;
    }

    emcmotCommand.command = EMCMOT_SET_JOINT_INPUT_SCALE;
    emcmotCommand.joint = joint;
    emcmotCommand.input_scale = input_scale;

    int retval = usrmotWriteEmcmotCommand(&emcmotCommand);

    if (emc_debug & EMC_DEBUG_CONFIG) {
        rcs_print("%s(%d, %.4f) returned %d\n", __FUNCTION__, joint, input_scale, retval);
    }
    return retval;
}
...
{
    CATCH_NAN(rtapi_isnan(home) || rtapi_isnan(offset) || rtapi_isnan(home_final_vel) || rtapi_isnan(search_vel) || rtapi_isnan(latch_vel));
    ...
    if (JointConfig[joint].SyncId >= 0) {
        if (joint < JointConfig[joint].SyncId) {
            // the joint with smaller SYNC_ID is GANTRY_MASTER
            emcmotCommand.flags |= HOME_GANTRY_MASTER;
            emcmotCommand.flags |= HOME_GANTRY_JOINT;
        } else {
            // the joint with greater SYNC_ID is GANTRY_SLAVE
            emcmotCommand.flags |= HOME_GANTRY_JOINT;
        }
    }
...
int emcJointSetSyncId(int joint, int sync_id)
{
    if (joint < 0 || joint >= EMCMOT_MAX_JOINTS) {
        rcs_print_error("%s (%s:%d) Invalid joint(%d)\n", __FILE__, __FUNCTION__, __LINE__,
                        joint);
        return -1;
    }
    if ((sync_id == joint) || (sync_id >= EMCMOT_MAX_JOINTS)) {
        rcs_print_error("%s (%s:%d) Invalid sync_id(%d) for joint(%d)\n", __FILE__, __FUNCTION__, __LINE__,
                        sync_id, joint);
        return -1;
    }

    if (sync_id < 0) {
        /* invalid sync_id, disable synchronized motion for this joint */
        JointConfig[joint].SyncEnable= 0;
        JointConfig[joint].SyncId= -1;
    } else {
        /* enable and specify synchronized joint */
        JointConfig[joint].SyncEnable= 1;
        JointConfig[joint].SyncId= sync_id;
    }
...
int emcJointAbort(int joint)
{
    ...
    if (JointConfig[joint].SyncEnable && (emcmotStatus.motion_state == EMCMOT_MOTION_FREE)) {
        // synchronize jog for gantry joint
        emcmotCommand.command = EMCMOT_JOINT_ABORT;
        emcmotCommand.joint = JointConfig[joint].SyncId;
...
int emcJogCont(int nr, double vel)
{
    ...
    if (JointConfig[nr].SyncEnable && (emcmotStatus.motion_state == EMCMOT_MOTION_FREE)) {
        ...
        emcmotCommand.joint = JointConfig[nr].SyncId;
        emcmotCommand.vel = vel;
        usrmotWriteEmcmotCommand(&emcmotCommand);
    }
...
{
    CATCH_NAN(rtapi_isnan(home.tran.x) || rtapi_isnan(home.tran.y) || rtapi_isnan(home.tran.z) ||
...
int emcTrajLinearMove(EmcPose end, int type, double vel, double ini_maxvel, double acc, double jerk,
                      int indexrotary)
{
    CATCH_NAN(rtapi_isnan(end.tran.x) || rtapi_isnan(end.tran.y) || rtapi_isnan(end.tran.z) ||
...
int emcSpindleOn(double speed, double css_factor, double offset)
{
    ...
    emcmotCommand.ini_maxvel = AxisConfig[localSpindleAxis].MaxVel;    
    emcmotCommand.acc = AxisConfig[localSpindleAxis].MaxAccel;
    emcmotCommand.ini_maxjerk = AxisConfig[localSpindleAxis].MaxJerk;
    return usrmotWriteEmcmotCommand(&emcmotCommand);
}
