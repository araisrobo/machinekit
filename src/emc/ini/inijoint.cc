...
/*
  ...
  emcJointSetInputScale(int joint, double input_scale);
...
static int loadJoint(int joint, EmcIniFile *jointIniFile)
{
    ...
    try {
        ...
        // set input_scale
        input_scale = 0;                   // default
        jointIniFile->Find(&input_scale, "INPUT_SCALE", jointString);
        if (0 != emcJointSetInputScale(joint, input_scale)) {
            return -1;
        }
