...
int ini_hal_init(void)
{
    ...
    for (int idx = 0; idx < EMCMOT_MAX_JOINTS; idx++) {
        ...
        MAKE_FLOAT_PIN_IDX(input_scale,HAL_IN,idx);
...
int check_ini_hal_items()
{
    ...
    for (int idx = 0; idx < EMCMOT_MAX_JOINTS; idx++) {
        ...
        if (CHANGED_IDX(input_scale,idx) ) {
            if (debug) SHOW_CHANGE_IDX(input_scale,idx);
            UPDATE_IDX(input_scale,idx);
            if (0 != emcJointSetInputScale(idx,NEW(input_scale[idx]))) {
                if (emc_debug & EMC_DEBUG_CONFIG) {
                    rcs_print("check_ini_hal_items:bad return value from emcJointSetInputScale\n");
                }
            }
        }
