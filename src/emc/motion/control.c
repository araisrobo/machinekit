	...
	*emcmot_hal_data->pause_state = PS_PAUSING;;
	...
	// on alternate queue  here, all motion stopped
		...
		*emcmot_hal_data->pause_jog_vel : emcmotCommand->ini_maxvel;
	    ...
	    emcmotConfig->vtp->tpSetId(emcmotQueue, emcmotStatus->id);
...
static void handle_usbmot_sync(void)
{
    ...
    if (emcmotStatus->update_pos_ack != 0)
    {
        ...
        for (joint_num = 0; joint_num < emcmotConfig->numJoints; joint_num++) {
            ...
            positions[joint_num] = joint->coarse_pos;
//            rtapi_print (
//                    _("(%s:%d): j[%d] risc_pos_cmd(%f) pos_cmd(%f) pos_fb(%f) free_pos_cmd(%f) motor_offset(%f)\n"), __FUNCTION__, __LINE__,
//                    joint_num,
//                    joint->risc_pos_cmd,
//                    joint->pos_cmd,
//                    joint->pos_fb,
...
static void get_pos_cmds(long period)
{
	...
	while (cubicNeedNextPoint(&(joints[0].cubic))) {
	    ...
	    result = emcmotConfig->vtk->kinematicsInverse(&emcmotStatus->carte_pos_cmd, positions,
						 &iflags, &fflags);
	    if(result == 0)
	    {
		/* copy to joint structures and spline them up */
		for (joint_num = 0; joint_num < emcmotConfig->numJoints; joint_num++) {
		    if(rtapi_isinf(positions[joint_num]))
		    {
    ...
    */
	...
	if ((!GET_JOINT_ACTIVE_FLAG(joint)) || (!GET_JOINT_HOMED_FLAG(joint)) || (!(joint->en_soft_limit))) {
	    continue;
        }
...
static void update_status(void)
{
    ...
    */
	...
	emcmotStatus->prim_dtg = emcmotQueue->distance_to_go; //FIXME: figure out how to tpGet...
