...
/* global kinematics flags; also for control.c*/
...
int inRange(EmcPose pos, int id, char *move_type)
{
    ...
    if (emcmotConfig->vtk->kinematicsInverse(&pos, joint_pos, &iflags, &fflags) < 0)
    {
	...
	if (!GET_JOINT_ACTIVE_FLAG(joint)) {
        ...
        if (joint->en_soft_limit) {
            ...
            if(rtapi_isinf(joint_pos[joint_num]))
            {
...
// remove after branch stabilization:
	...
	case EMCMOT_SET_JOINT_BACKLASH:
        ...
        case EMCMOT_SET_JOINT_INPUT_SCALE:
            /* set the input_scale for the joint */
            /* can be done at any time */
            rtapi_print_msg(RTAPI_MSG_DBG, "SET_JOINT_INPUT_SCALE");
            rtapi_print_msg(RTAPI_MSG_DBG, " %d", joint_num);
            emcmot_config_change();
            if (joint == 0) {
                break;
            }
            joint->scale_recip = 1.0 / emcmotCommand->input_scale;
            break;
...
/*! \todo Another #if 0 */
	...
	case EMCMOT_SET_AXIS_POSITION_LIMITS:
        ...
        default:
            reportError(_("UNKNOWN emcmotCommand(%d)"), emcmotCommand->command);
            rtapi_print_msg(RTAPI_MSG_ERR, "UNKNOWN emcmotCommand(%d)", emcmotCommand->command);
            emcmotStatus->commandStatus = EMCMOT_COMMAND_UNKNOWN_COMMAND;
            break;
	}			/* end of: command switch */
	if (emcmotStatus->commandStatus != EMCMOT_COMMAND_OK) {
	    rtapi_print_msg(RTAPI_MSG_ERR, "ERROR: emcmotCommand(%d) emcmotStatus(%d)", emcmotCommand->command, emcmotStatus->commandStatus);
	}
	rtapi_print_msg(RTAPI_MSG_DBG, "\n");
	/* synch tail count */
