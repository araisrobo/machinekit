...
int sendJointSetInputScale(int joint, double input_scale)
{
    EMC_JOINT_SET_INPUT_SCALE emc_joint_set_input_scale_msg;

    emc_joint_set_input_scale_msg.joint = joint;
    emc_joint_set_input_scale_msg.input_scale = input_scale;
    emcCommandSend(emc_joint_set_input_scale_msg);
    if (emcWaitType == EMC_WAIT_RECEIVED) {
        return emcCommandWaitReceived();
    } else if (emcWaitType == EMC_WAIT_DONE) {
        return emcCommandWaitDone();
